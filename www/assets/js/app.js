...
`;
  ...
  var N3Lexer = class {
    ...
    _unescape(item2) {
      let invalid = false;
      const replaced = item2.replace(escapeSequence, (sequence, unicode4, unicode8, escapedChar) => {
  ...
  var N3Parser = class {
    ...
    _readListItem(token) {
      let item2 = null, list = null, next = this._readListItem;
      ...
      switch (token.type) {
        case "[":
          this._saveContext(
            ...
            this.RDF_FIRST,
            this._subject = item2 = this._factory.blankNode()
          );
        ...
        case "literal":
          if (token.prefix.length === 0) {
            ...
            item2 = this._factory.literal(token.value, this._factory.namedNode(token.prefix));
            next = this._getContextEndReader();
          }
          break;
        case "{":
          ...
          return this._readSubject;
        default:
          if ((item2 = this._readEntity(token)) === void 0)
            return;
      }
      if (list === null)
      ...
      if (item2 !== null) {
        if (this._n3Mode && (token.type === "IRI" || token.type === "prefixed")) {
          this._saveContext("item", this._graph, list, this.RDF_FIRST, item2);
          this._subject = item2, this._predicate = null;
          return this._getPathReader(this._readListItem);
        }
        this._emit(list, this.RDF_FIRST, item2, this._graph);
      }
      return next;
    }
      ...
      switch (token.type) {
        ...
        default:
          ...
          if (parent && parent.type === "item") {
            const item2 = this._subject;
            this._restoreContext("item", token);
            this._emit(this._subject, this.RDF_FIRST, item2, this._graph);
          }
...
`);
    ...
    match(path, options) {
      ...
      for (let route of this.routeInfo) {
        ...
        if (matches && matches.length) {
          ...
          args.result = route.action.call(route, params2);
    ...
    handleEvents() {
      ...
      globalThis.document.addEventListener("click", (evt) => {
        ...
        if (link && link.pathname && link.hostname == globalThis.location.hostname && !link.link && !link.dataset.simplyCommand) {
          ...
          if (this.has(path)) {
            let params2 = this.runListeners("goto", { path });
            if (params2.path) {
              this.goto(params2.path);
            }
            evt.preventDefault();
            return false;
          }
        ...
        if (context.element.dataset.transform) {
          context.element.dataset.transform.split(" ").filter(Boolean).forEach((t) => {
            if (this.options.transformers[t]) {
              transformers2.push(this.options.transformers[t]);
            } else {
  ...
  var everything_default2 = simply;
  ...
  var zett = everything_default2.app({
    routes: {
      ...imports.routes
    },
    keys: {
    ...
    actions: {
      ...imports.actions,
      openLink: async function(url2) {
        url2 = new URL(url2);
        const file = await this.actions.addFile(url2);
        if (!file) {
          return;
        }
        if (url2.hash) {
      ...
      addFile: async function(url2) {
        const file = await solidApi.fetch(url2, this.state.login);
        const data = solidApi.mergeSubjects(file.data, url2, file.prefixes);
